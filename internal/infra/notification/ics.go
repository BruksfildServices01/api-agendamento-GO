...
func buildAppointmentICS(input domain.AppointmentConfirmedInput) string {
	...
	b.WriteString("DESCRIPTION:Agendamento confirmado na barbearia " + input.BarbershopName + "\r\n")
...
func buildRescheduledICS(input domain.AppointmentRescheduledInput) string {
	startUTC := input.NewStartTime.UTC().Format("20060102T150405Z")
	endUTC := input.NewEndTime.UTC().Format("20060102T150405Z")

	uid := fmt.Sprintf(
		"%s-%d@corteon",
		strings.ReplaceAll(input.ClientEmail, "@", "_"),
		input.NewStartTime.Unix(),
	)
	...
	b.WriteString("DTSTART:" + startUTC + "\r\n")
	b.WriteString("DTEND:" + endUTC + "\r\n")
	b.WriteString("SUMMARY:Agendamento remarcado\r\n")
	b.WriteString("DESCRIPTION:Agendamento remarcado na barbearia " + input.BarbershopName + "\r\n")
	b.WriteString("END:VEVENT\r\n")
